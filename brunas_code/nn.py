...
for seed in seeds:
    ...
    def train(optimizer_name, model, train_loader):
        ...
        save_path = os.path.join(f"{PATH}/data", f"nn--{optimizer_name}-seed{seed}.pt")
    ...
    def test(optimizer_name, model, test_loader):
        ...
        model_path = os.path.join(f"{PATH}/data", f"nn--{optimizer_name}-seed{seed}.pt")
...
plt.title(f"Neural Network on MNIST - Mean ± Std (n={len(seeds)})")
plt.ylim([min_y, max_y])  # Set the dynamic range

plot_path = os.path.join(f"{PATH}/results", f"nn_accuracy.png")
plt.savefig(plot_path)
plt.close()

if not only_test: 
    with open("nn_val_accuracies.txt", "a") as file:
